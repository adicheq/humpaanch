...
export default function VotingCard({
  ...
  if (options.length === 0) {
    return (
      <div className="bg-[var(--bg-secondary)] rounded-xl p-4 text-[var(--text-muted)] text-sm text-center">
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        {votingOpen ? (
          ...
          <span className="text-xs text-[var(--text-muted)] font-medium">Voting closed</span>
        )}
      </div>
      ...
      {options.map((option, index) => {
        ...
        return (
          <motion.button
            ...
            initial="hidden"
            animate="show"
            whileTap={votingOpen ? { scale: 0.97 } : undefined}
            ...
            className={`w-full text-left p-4 rounded-xl transition-all ${
              isWinner
                ...
                : isMyChoice
                ? "bg-[var(--text-primary)]/10 border-2 border-[var(--text-primary)]/40"
                : "bg-[var(--bg-secondary)] border-2 border-transparent active:bg-[var(--bg-button)]"
